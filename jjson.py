...
from src.utils.convertor import dict2namespace
...
def j_dumps(
    ...
    Args:
        data (Union[dict, SimpleNamespace, List[dict], List[SimpleNamespace]]): JSON objects or SimpleNamespace objects to be dumped.
        file_path (Union[Path, str], optional): Path to the output file. If None, returns the JSON data as a dictionary. Defaults to None.
    ...
    Returns:
        Union[dict, None]: The JSON data as a dictionary if successful, or None if an error occurred.
...
def j_loads(
    jjson: Union[Path, dict, str], ordered: bool = False, exc_info: bool = True
) -> Union[dict, List[dict], bool, None]:
    """Load JSON or CSV data from a file, directory, or string.

    Args:
        jjson (Union[Path, dict, str]): Path to a file, directory, or JSON data as a string, or JSON object.
    ...
    Returns:
        Union[dict, List[dict], bool, None]: Returns a dictionary or list of dictionaries if successfully loaded. Returns `False` if an error occurred. Returns `None` if `jjson` is not found or cannot be read.
                    ...
                    for root, dirs, files in os.walk(jjson_path):
                        for file in files:
                            ...
                            if file.endswith(".json"):
                                json_data.extend(json.load(file_path))
...
def j_loads_ns(
    ...
    Args:
        jjson (Union[Path, SimpleNamespace, dict, str]): Path to a file, directory, or JSON data as a string, or JSON object.
        ordered (bool, optional): If `True`, returns OrderedDict instead of a regular dict to preserve element order. Defaults to `False`.
        exc_info (bool, optional): If `True`, logs exceptions with traceback. Defaults to `True`.

    Returns:
        Union[SimpleNamespace, List[SimpleNamespace], None]: Returns SimpleNamespace or a list of SimpleNamespace if successfully loaded and converted. Returns `None` if data cannot be loaded.
    ...
    """
    data = j_loads(jjson, ordered, exc_info)
    if data:
        return dict2namespace(data) if isinstance(data, dict) else [dict2namespace(item) for item in data if isinstance(item, dict)]
    return 

def collect_and_merge_json_files(directory: Path):
    """Collect and merge JSON files from a directory into a single JSON file.
    ...
    try:
        ...
        if json_data:
            merged_data = merge_dicts(json_data)
            j_dumps(merged_data, file_path=Path(directory, 'merged.json'))
    except Exception as ex:
        logger.error(f"Error collecting and merging JSON files: {directory}", ex, True)
